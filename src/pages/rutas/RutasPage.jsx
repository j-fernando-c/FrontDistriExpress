...
import { getAllRutas, createRuta, updateRuta, deleteRuta } from '../../api/rutasService';
 ...
 * 
 * Permite crear, leer, actualizar y eliminar registros de Rutas.
...
const RutasPage = () => {
  ...
  // Fetch data
  const { data: items, loading, error, refetch } = useFetch(() => getAllRutas(), {
    autoFetch: true,
  });
  ...
  const handleCreate = async () => {
    ...
    await execute(async () => {
      ...
      setFormData({ nombre: '', origen: '', destino: '', distancia: '', estado: '' });
      toggleModal();
      refetch();
    });
  };
  ...
  // Form validation
  const validateForm = () => {
    if (!formData.nombre || !formData.origen || !formData.destino || !formData.distancia || !formData.estado) {
      return false;
    }
    return true;
  };
  ...
  return (
    <Card title="Gestión de Rutas">
      ...
      {/* Create/Edit Modal */}
      <Modal
        title={editingId ? 'Editar Ruta' : 'Crear Ruta'}
        isOpen={showModal}
        onClose={() => {
          toggleModal();
          setFormData({ nombre: '', origen: '', destino: '', distancia: '', estado: '' });
