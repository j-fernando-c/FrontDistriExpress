...
import { getAllCompras, createCompra, updateCompra, deleteCompra } from '../../api/comprasService';
 ...
 * 
 * Permite crear, leer, actualizar y eliminar registros de Compras.
...
const ComprasPage = () => {
  ...
  // Fetch data
  const { data: items, loading, error, refetch } = useFetch(() => getAllCompras(), {
    autoFetch: true,
  });
  ...
  const handleCreate = async () => {
    ...
    await execute(async () => {
      ...
      setFormData({ proveedor: '', fecha: '', total: '', estado: '' });
      toggleModal();
      refetch();
    });
  };
  ...
  // Form validation
  const validateForm = () => {
    if (!formData.proveedor || !formData.fecha || !formData.total || !formData.estado) {
      return false;
    }
    return true;
  };
  ...
  return (
    <Card title="Gestión de Compras">
      ...
      {/* Create/Edit Modal */}
      <Modal
        title={editingId ? 'Editar Compra' : 'Crear Compra'}
        isOpen={showModal}
        onClose={() => {
          toggleModal();
          setFormData({ proveedor: '', fecha: '', total: '', estado: '' });
